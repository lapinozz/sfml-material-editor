...
#include "mls/variant-emplace.hpp"
...
struct ProjectEditor
{
    ...
    void initImgui()
    {
        ...
        io.Fonts->Clear();
        io.Fonts
            ->AddFontFromMemoryCompressedTTF(Cousine_compressed_data, Cousine_compressed_size, baseFontSize * sizeScalar)
            ->Scale = 1.f / sizeScalar;
        ...
        io.Fonts
            ->AddFontFromMemoryTTF((void*)s_fa_solid_900_ttf, sizeof(s_fa_solid_900_ttf), faFontSize * sizeScalar, &font_cfg, icons_ranges)
            ->Scale = 1.f / sizeScalar;
    ...
    bool close()
    {
        if (isDirty())
        {
            const auto status = Dialog::Show(Dialog::Type::YesNoCancel,
                                             "Unsaved Changes",
                ...
                if (ImGui::Button("..."))
                {
                    ...
                    else
                    {
                        Dialog::Show(Dialog::Type::Error,
                                     "Save Project First",
                                     "Save the project to a file to select a relative path.");
                    }
                }
            }
            else if (textureReference.type == TextureReference::Type::Embedded)
    ...
    void drawMainWindow()
    {
        ...
        static ImGuiTableFlags flags = ImGuiTableFlags_SizingStretchSame | ImGuiTableFlags_Resizable |
                                       ImGuiTableFlags_BordersInnerV | ImGuiTableFlags_ContextMenuInBody;
        ...
        if (ImGui::BeginTabBar("middle_tabs",
