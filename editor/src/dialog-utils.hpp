...
Status Show(Type type, std::string title, std::string text)
{
    ...
    {
        NvdQuestionBox* dialog = nvd_dialog_question_new(title.c_str(), text.c_str(), NVD_YES_NO_CANCEL);
    ...
    {
        NvdDialogBox* dialog = nvd_dialog_box_new(title.c_str(), text.c_str(), NVD_DIALOG_ERROR);
